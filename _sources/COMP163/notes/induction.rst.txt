...
#. Start with :math:`L(k+1)` and see if you can get to :math:`R(k+1)`, remembering that in all likelihood :math:`L(k+1) = L(k) + \Lambda(k+1)`, where :math:`\Lambda(k+1)` is some expression for the :math:`(k+1)`-th term of :math:`L`. If so, the problem becomes an exercise in showing that :math:`L(k)+\Lambda(k+1)=R(k+1)`.